 ...
 */
public class SyncStoreBankNew {

    private static final Log logger = LogFactory.getLog(SyncStoreBankNew.class);
    ...
    private void insertOrUpdateBankInfo(ArrayList<String[]> csvFileList) {
        ...
        try {
            ...
            for (int row = 0; row < csvFileList.size(); row++) {
                ...
                String insertSql = String.format("insert into lp_store_bank (store_id,bank_no,username,is_public_account, open_bank, super_bank_no,united_bank_no) values" +
                                "('%s','%s','%s','%s','%s','%s','%s')",
                ...
                String updateSql = String.format("update lp_store_bank " +
                                ...
                                "open_bank='%s'," +
                                "super_bank_no='%s'," +
                                "united_bank_no='%s', update_time= current_timestamp " +
                                " where store_id='%s' AND is_unbind = 0 ",
                        bank_no, username, is_public_account, open_bank, super_bank_no, united_bank_no, store_id);


                if (count > 0) {
                    //update
                    DBBankNewUtilsTemplate.update(updateSql);
                    updateRows++;
                } else {
                    //insert
                    DBBankNewUtilsTemplate.update(insertSql);
                    insertRows++;
                }
            }
